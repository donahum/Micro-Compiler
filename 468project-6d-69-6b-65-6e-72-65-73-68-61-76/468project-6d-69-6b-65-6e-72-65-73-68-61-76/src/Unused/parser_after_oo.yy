...
%{
    ...
    int scope_iterator = 1;
    int decl_exists = 0;
    int param_decl_exists = 0;
...
decl              : string_decl {$<s_entry>$ = $<s_entry>1;  (*$<s_entry>$).printSTE();  decl_exists = 1;} decl; 
                    | var_decl  {$<s_entry>$ = $<s_entry>1;  (*$<s_entry>$).printSTE();  decl_exists = 1;} decl;
                    | %empty         {decl_exists = 0;/*std::cout << "Found nothing in decl\n";*/};
...
param_decl_list   : param_decl {param_decl_exists = 1;//std::cout << "param_decl_list\n";
                                /*currTable.addEntry($<s_entry>1); */
                               } param_decl_tail 
                    | %empty   {param_decl_exists = 0;/*std::cout << "empty param_decl_list\n";*/};

param_decl        : var_type id {//std::cout << "param_decl " << $<s>1 << $<s>2 << std::endl;  
                                 SymbolTableEntry newEntry = SymbolTableEntry($<s>2, $<s>1);
                                 if(param_decl_exists){funcTable.addEntry(newEntry);}
                                 /*funcTable.printST();*/  
                                 /*std::cout << $<s>1 << " " << $<s>2 << "\n";*/
                                };
...
func_decl         : _FUNCTION any_type id {SymbolTable funcTable = SymbolTable();} _OPAREN param_decl_list {//std::cout << $<s>3;
                                                                                                            funcTable.setScope($<s>3);//funcTable.printST();
...
if_stmt           : _IF {
                    ...
                    _OPAREN cond _CPAREN decl {//(*$<s_entry>4).printSTE();
                                               if(decl_exists){ifTable.addEntry($<s_entry>4);}
                                               //ifTable.addEntry($<s_entry>4);
                                               //std::cout << ifTable.getScope();
                                               //std::cout << "found an if decl";
                                             ;} 
                    stmt_list {std::string ifName = std::to_string(static_cast<long long>(scope_iterator)); 
                               ++scope_iterator;
                               ifTable.setScope("Block " + ifName);
                               tableVector.push_back(ifTable);
                               ifTable = SymbolTable();
                              } 
                    else_part {
...
else_part         : _ELSE {
                    ...
                    decl  {
                           if(decl_exists){ifTable.addEntry($<s_entry>1);}
                          }
                    stmt_list {std::string ifName = std::to_string(static_cast<long long>(scope_iterator)); 
                               ++scope_iterator;
                               elseTable.setScope("Block " + ifName);
                               tableVector.push_back(elseTable);
                               elseTable = SymbolTable();}
                    | %empty;
...
while_stmt        : _WHILE {
                    ...
                    _OPAREN cond _CPAREN decl {
                                               if(decl_exists){whileTable.addEntry($<s_entry>1);}
                                              } 
                    stmt_list {std::string ifName = std::to_string(static_cast<long long>(scope_iterator)); 
                               ++scope_iterator;
                               whileTable.setScope("Block " + ifName);
                               tableVector.push_back(whileTable);
                               whileTable = SymbolTable();}
                   _ENDWHILE;
